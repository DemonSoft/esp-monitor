## ВНИМАНИЕ!
============================================================
Esp32-CAM  должен быть подключен к внешнему питанию 5.5V !!!
Ни в коем случае, не используйте питание программатора!!!

Правильная схема подключения (Внешнее питание + FTDI):
Внешний блок питания:
+5V (или 5.5V) → Пин 5V на ESP32-CAM.
- (GND) → Пин GND на ESP32-CAM.
FTDI-программатор:
Пин 5V / VCC на FTDI → ОТКЛЮЧИТЬ! (Не подключать никуда).
Пин GND на FTDI → ОБЯЗАТЕЛЬНО на пин GND ESP32-CAM (туда же, куда подключен минус внешнего БП).
Пин TX на FTDI → Пин U0R (RX) на ESP32-CAM.
Пин RX на FTDI → Пин U0T (TX) на ESP32-CAM.
============================================================
Для того, чтоб сервер предоставил возможность выбора режима камеры, необходимо, чтоб в MDNS имени устройства содержалась подстрока "-cam"
============================================================
Датчик камеры подвержен перегреву при частой съемке, даже 1 раз в 5 секунд, что приводит к выходу его из строя. Поскольку, для отладки работы микроконтроллера, необходима работа его длительное время, режим по-умолчанию установлен в IDLE (без фотосъемки). Для запуска камеры требуется отправить конфигурацию с сервера в одном из предустановленных режимов (1-3).
============================================================
К GPIO14 подключен фоторезистор, который работает в цифровом режиме (0 - светло, 1 - темно). Предполагается, что съемка будет происходить только при условии, что фоторезистор детектирует освещение.
============================================================



## Алгоритм работы esp32 микроконтроллера.
-------------------

1. При включении микроконтроллеера, если в файловой системе отсуствует файл config.json, микроконтроллер поднимает точку доступа c ssid = "esp-setup" и паролем "1qazxsw2"; запускает веб-сервер для обработки REST запроса по адресу 192.168.4.1 на 80-м порту. 
